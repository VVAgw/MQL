/*
Расчет всех пар по разнице между ценой и средней
*/
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,ChartIndicatorName(0,ChartWindowFind(),0)+" ("+string(fast_ema_period)+","+string(slow_ema_period)+","+string(signal_period)+"), усреднение ("+string(ema)+")");
                ...
                const int &spread[])
{
   ...
   for (int i = limit; i < rates_total; i ++)
   {
      ...
      bufer1[i] = closePara1[i] - emaPara1[i];
      bufer2[i] = closePara2[i] - emaPara2[i];
   }
